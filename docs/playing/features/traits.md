...
| Snitch | Informs you about events among your mercs (base 50% chance, modified by relationships and leadership); intended to build your reputation over time (currently inverted by a bug — see [Reputation](reputation.md)); +1 hearing range; can work undercover in prisons as a covert interrogator (3× interrogation effectiveness; the INI's separate guard-strength multiplier is loaded but unused in current code) — see [Snitches & informants](snitches.md). |
